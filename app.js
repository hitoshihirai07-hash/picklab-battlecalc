(() => {
  ...
  function createSearchBox({placeholder, getOptions, onPick, formatLabel}) {
    ...
    const list = el("div", {class:"card", style:"display:none; position:absolute; left:0; right:0; top:calc(100% + 6px); padding:6px;"});
...
// --- Dex loading ---
  ...
  function renderMonCard(side, idx, mon) {
    ...
    const itemBox = createSearchBox({
      ...
      onPick: (o) => {
        if (trySetItem(side, idx, mon, o.en)) {
          renderAll();
        }
      },
      formatLabel: (o) => `${o.ja}`
    ...
    natureSel.addEventListener("change", e => { mon.nature = e.target.value; updateComputed(); });
        ...
        const usedItems = state.ui.normalRules ? getUsedItems(side, idx) : new Set();

    // Set suggestions
    const setBox = el("div", {class:"field"});
    const setTitle = el("div", {class:"small"}, "セット候補（クリックで反映）");
    const chips = el("div", {class:"chips"});
    const setSuggestions = getSetSuggestions(mon.speciesId, 3, usedItems);
    if (!setSuggestions.length) {
      ...
      for (const s of setSuggestions) {
        ...
        chip.addEventListener("click", () => {
          const r = applySetToMon(mon, s, {side, idx});
          renderAll();
          if (r && r.itemNote) setStatus(r.itemNote, "note");
        });
    ...
    // Recommended item chips（通常ルール適用中は同一持ち物を除外）
    let recItems = getRecommendedItems(mon.speciesId, 16);
    if (state.ui.normalRules && usedItems.size) {
      recItems = recItems.filter(it => it && !usedItems.has(it));
    }
    if (recItems.length < 8) {
      const extra = getGenericItemCandidates(usedItems, 16);
      for (const it of extra) {
        if (it && !recItems.includes(it)) recItems.push(it);
      }
    }
    recItems = recItems.slice(0, 8);

    const itemRecBox = el("div", {class:"chips"});
    if (recItems.length) {
      for (const itEn of recItems) {
        const ja = fmtItem(itEn);
        const chip = el("span", {class:"chip", title: itEn}, ja);
        chip.addEventListener("click", () => {
          if (trySetItem(side, idx, mon, itEn)) {
            renderAll();
          }
        });
  ...
  function fmtItem(en){
    return state.jpItemByEn.get(en) || en;
  }

  function getUsedItems(side, excludeIdx=null){
    const used = new Set();
  ...
  function isItemDuplicate(side, idx, itemEn){
    if (!itemEn) return false;
    const team = state.teams?.[side] || [];
    for (let i=0;i<team.length;i++){
      if (i === idx) continue;
      if (team[i]?.item === itemEn) return true;
    }
    return false;
  }

  function trySetItem(side, idx, mon, itemEn){
    if (state.ui.normalRules && itemEn && isItemDuplicate(side, idx, itemEn)) {
      setStatus(`同じ持ち物は複数持てません（${fmtItem(itemEn)}は既に使われています）。`, "note");
      return false;
    }
    mon.item = itemEn || "";
    return true;
  ...
  function getGenericItemCandidates(excludeSet, limit=8){
    const out = [];
    for (const it of GENERIC_ITEMS) {
      if (!excludeSet || !excludeSet.has(it)) out.push(it);
      if (out.length >= limit) break;
    }
    return out;
  }

  function pickAlternativeItem(speciesId, usedSet, avoidItem){
    const rec = getRecommendedItems(speciesId, 30);
    for (const it of rec) {
      if (!it) continue;
      if (it === avoidItem) continue;
      if (usedSet && usedSet.has(it)) continue;
      return it;
    }
    const fallback = getGenericItemCandidates(usedSet, 30);
  ...
  function getSetSuggestions(speciesId, limit=3, excludeItems=null){
    ...
    const entries = Object.entries(setsFor);
    const out = [];
    const usedNames = new Set();

    // 1) Prefer sets whose item doesn't conflict (通常ルールの同一持ち物回避)
    if (excludeItems && excludeItems.size) {
      for (const [setName, setObj] of entries) {
        const it = setObj?.item;
        if (it && excludeItems.has(it)) continue;
    ...
    // 2) Fill remaining (fallback)
    for (const [setName, setObj] of entries) {
      if (usedNames.has(setName)) continue;
      out.push({name:setName, data:setObj});
      if (out.length >= limit) break;
    }
    return out;
  ...
  function applySetToMon(mon, set, ctx){
    const s = set.data || {};
    if (!mon.speciesId) return null;
    ...
    if (s.item) {
      const desired = s.item;
      if (side && state.ui.normalRules && isItemDuplicate(side, idx, desired)) {
        const used = getUsedItems(side, idx);
        const alt = pickAlternativeItem(mon.speciesId, used, desired);
        if (alt) {
          mon.item = alt;
          itemNote = `持ち物重複回避で「${fmtItem(alt)}」に変更しました。`;
        } else {
          mon.item = "";
          itemNote = `持ち物「${fmtItem(desired)}」は重複のため未設定にしました。`;
        }
      } else {
        mon.item = desired;
      }
  ...
  // --- BDC handoff (Pick Lab -> /calc/) ---
  function saveCalcStateAndGo(){
    const payload = {
      savedAt: new Date().toISOString(),
      app: {
        v: 1,
